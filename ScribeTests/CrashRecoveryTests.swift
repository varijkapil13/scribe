...
final class CrashRecoveryTests: XCTestCase {
    ...
    func testRecoverFinalisesDanglingSessionsWithSegments() throws {
        let session = try store.createSession(title: "Dangling")
    ...
    func testRecoverHandlesSessionWithNoSegments() throws {
        let session = try store.createSession(title: "Empty crash")
    ...
    func testRecoverIgnoresAlreadyEndedSessions() throws {
        let s1 = try store.createSession(title: "Done")
        ...
        let s2 = try store.createSession(title: "Crashed")
    ...
    func testRecoverIsIdempotent() throws {
        let session = try store.createSession(title: "Once")
